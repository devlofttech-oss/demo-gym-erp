      ...
      {showForm && (
        <div className={cardBase + ' border border-primary/20'}>
          ...
          <div className="flex items-center gap-3 pt-2 border-t border-outline-variant/20">
            ...
            <a
              href={form.title.trim() && form.message.trim() ? announcementWaUrl() : undefined}
              target="_blank"
              rel="noopener noreferrer"
              aria-disabled={!(form.title.trim() && form.message.trim())}
              className={`bg-[#25D366] hover:bg-[#1ebe5d] text-white px-4 py-2.5 rounded-lg font-medium transition-colors shadow-sm flex items-center gap-2 text-sm ${!(form.title.trim() && form.message.trim()) ? 'opacity-50 pointer-events-none' : ''}`}
            >
