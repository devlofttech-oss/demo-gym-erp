...
class _PaymentScreenState extends State<PaymentScreen> {
  ...
  // Fully paid = no outstanding balance. paidFees is deliberately not part of
  // this: a zero-price plan, or a member imported with no fees recorded, has
  // paidFees 0 and still owes nothing.
  bool get _isFullyPaid =>
      _selected != null && asNum(_selected!['balanceFees']) == 0;

  bool get _nothingToCollect => _isFullyPaid && _outstandingBase == 0;

  num get _discountedTotal =>
      (_totalFees - _discountAmt).clamp(0, double.infinity);
  num get _outstandingBase => asNum(_selected?['balanceFees']) > 0
  ...
  Map<String, dynamic> _matchPlan(String planName) {
    for (final p in _plans) {
      if (p['name'] == planName) return p;
    }
    for (final p in _plans) {
      final n = p['name'] as String?;
      if (n != null && n.isNotEmpty && planName.contains(n)) return p;
    }
    return _plans.isNotEmpty ? _plans.first : {};
  }
  ...
  void _loadMember(String id) {
    ...
    if (m.isEmpty) return;
    _selected = m;
    final fullyPaid = asNum(m['balanceFees']) == 0;
    if (!fullyPaid) {
      ...
      final matched = _matchPlan((m['planName'] as String?) ?? '');
  ...
  Future<void> _pickDate(String which) async {
    final init =
        DateTime.tryParse(which == 'active' ? _planActiveFrom : _expiryDate) ??
        DateTime.now();
    ...
    setState(() {
      if (which == 'active') {
        _planActiveFrom = iso;
        _expiryDate = addMonths(iso, _durationMonths);
      } else
        _expiryDate = iso;
    });
  }
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            Text(
              'Process a payment and activate the member plan',
              ...
              children: [
                    ...
                    children: [
                      ...
                      TextField(
                        ...
                        decoration: _dec().copyWith(
                          hintText: 'e.g. Renewal, Annual offer',
                        ),
                      ),
                      const Divider(height: 32),
                ...
                SizedBox(
                  ...
                  child: FilledButton.icon(
                    ...
                    onPressed: _saving || _nothingToCollect ? null : _submit,
                    icon: _saving
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Pill(
                  m['status']?.toString() ?? 'Unknown',
                  fg: active ? TW.green700 : TW.rose600,
                  bg: active ? TW.green100 : TW.rose100,
                ),
              ],
            ),
          ),
          Padding(
            ...
            child: Row(
              children: [
                ...
                _infoCol(
                  'Outstanding',
                  bal > 0 ? rupees(bal) : 'Cleared',
                  color: bal > 0 ? TW.rose500 : TW.green600,
                ),
              ],
            ),
          ),
  ...
  Widget _fullyPaidBanner() {
    return Container(
      ...
      decoration: BoxDecoration(
        color: TW.green50,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(color: TW.green200),
      ),
      child: Row(
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: const [
                ...
                SizedBox(height: 2),
                Text(
                  'No outstanding balance. Select a new plan below to record a renewal.',
                  style: TextStyle(color: TW.green600, fontSize: 12),
                ),
              ],
            ),
          ),
              ...
              child: const Row(
                ...
                children: [
                  ...
                  Text(
                    ...
                    style: TextStyle(color: TW.amber700, fontSize: 11),
                  ),
                ],
              ),
            ),
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              const SizedBox(width: 8),
              Text(
                _planName.isNotEmpty ? _planName : '—',
                style: TextStyle(
                  color: c.onSurfaceVariant,
                  fontWeight: FontWeight.w600,
                ),
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          isExpanded: true,
          hint: Text(
            _plans.isEmpty ? 'No plans' : 'Select plan',
            style: TextStyle(color: c.onSurfaceVariant),
          ),
          dropdownColor: c.surfaceContainerLowest,
          style: TextStyle(
                ...
                child: TextField(
                  ...
                  onChanged: (v) {
                    ...
                    setState(() {
                      ...
                      _discountAmtCtrl.text = _discountAmt > 0
                          ? _discountAmt.toStringAsFixed(0)
                          : '';
                    });
                  },
                ...
                child: TextField(
                  ...
                  onChanged: (v) {
                    ...
                    setState(() {
                      _discountAmt = amt.clamp(0, _totalFees);
                      _discountPct = pct.clamp(0, 100);
                      _discountPctCtrl.text = _discountPct > 0
                          ? _discountPct.toStringAsFixed(1)
                          : '';
                    });
                  },
                  style: TextStyle(color: c.onSurface, fontSize: 13),
            ...
            children: [
              ...
              Container(
                ...
                child: hasDiscount
                    ...
                    : Text(
                        _totalFees > 0 ? rupees(_totalFees) : '—',
                        style: TextStyle(
                          color: c.onSurfaceVariant,
                          fontWeight: FontWeight.w600,
                          fontSize: 13,
  ...
  Widget _summary() {
    final c = context.c;
    final days = ((DateTime.tryParse(_expiryDate) ?? DateTime.now())
        .difference(DateTime.tryParse(_planActiveFrom) ?? DateTime.now())
        .inDays);
    return Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  ...
                  runSpacing: 4,
                  children: [
                    if (_selected != null && asNum(_selected!['paidFees']) > 0)
                      _sum(
                        'Already Paid',
                        rupees(asNum(_selected!['paidFees'])),
                        TW.green600,
                      ),
                    if (_discountAmt > 0)
                      _sum('Discount', '−${rupees(_discountAmt)}', TW.green600),
                ...
                const SizedBox(height: 6),
                Text(
                  'Plan: $_planActiveFrom → $_expiryDate ($days days) · $_paymentMode',
                  style: TextStyle(color: c.onSurfaceVariant, fontSize: 12),
                ),
              ],
            ),
  ...
  Widget _sum(String k, String v, Color color) => RichText(
    text: TextSpan(
      children: [
        TextSpan(
          text: '$k: ',
