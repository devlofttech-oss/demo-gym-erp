...
class _PaymentsScreenState extends State<PaymentsScreen> {
  ...
  num get _totalRevenue =>
      _payments.fold<num>(0, (s, p) => s + asNum(p['amount']));
  ...
  List<Map<String, dynamic>> get _filteredPayments {
    ...
    return _payments.where((p) {
      ...
      return ((p['memberName'] as String?)?.toLowerCase().contains(term) ??
              false) ||
          ((p['planName'] as String?)?.toLowerCase().contains(term) ?? false) ||
          ((p['paymentMode'] as String?)?.toLowerCase().contains(term) ??
              false);
    }).toList();
  }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          PageHeader(
            'Payments',
            'Track payments, dues, and expired memberships.',
            trailing: _recordBtn(),
          ),
  ...
  List<Widget> _paymentsTab() {
    ...
    final thisMonthSum = _payments
        .where((p) => _thisMonth(p['date']))
    ...
    final activeModes = _payModes
        .where((m) => (modeBreakdown[m] ?? 0) > 0)
        .toList();
    ...
    return [
      ...
      if (_loading)
        ...
        const KEmpty(icon: MSym.receiptLong, message: 'No payments found')
      else
        ...filtered.map(_paymentCard),
    ];
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: active ? c.primary : c.outlineVariant.withValues(alpha: 0.3),
          ),
        ),
        child: Text(
          label,
  ...
  List<Widget> _duesTab() {
    ...
    return [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      '${_duesMembers.length} member${_duesMembers.length > 1 ? 's' : ''} with expired or missing memberships',
                      style: const TextStyle(
                        color: TW.rose700,
                        fontWeight: FontWeight.w600,
                      ),
                    ...
                    const Text(
                      'Send WhatsApp reminders or renew their plans directly.',
                      style: TextStyle(color: TW.rose600, fontSize: 13),
                    ),
                  ],
                ),
              ),
      ...
      if (_loading)
        ...
        const KEmpty(
          icon: MSym.checkCircle,
          iconColor: TW.emerald500,
          message: 'No dues! All members are up to date.',
        )
      else
        ...filtered.map(_duesCard),
    ];
  }
          ...
          children: [
            Row(
              children: [
                ...
                if (overdue != null)
                  Pill('${overdue}d overdue', fg: TW.rose600, bg: TW.rose50),
              ],
            ),
            const SizedBox(height: 12),
            _kv(
              'Last Plan',
              (m['planName'] as String?)?.isNotEmpty == true
            ...
            Row(
              children: [
                ...
                Text(
                  m['expiryDate'] != null ? fmtDate(m['expiryDate']) : '—',
                  style: const TextStyle(
                    color: TW.rose600,
                    fontWeight: FontWeight.w500,
                  ),
                ...
                return Row(
                  children: [
                    ...
                    Text(
                      bal > 0 ? rupees(bal) : '—',
                      style: TextStyle(
                        color: bal > 0 ? TW.rose600 : c.onSurfaceVariant,
                        fontWeight: bal > 0 ? FontWeight.w600 : FontWeight.w400,
                      ),
                    ),
                  ],
                );
                  ...
                  child: FilledButton.icon(
                    ...
                    onPressed: () async {
                      await Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (_) =>
                              PaymentScreen(memberId: m['id'] as String?),
                        ),
                      );
                      setState(() => _loading = true);
                      _fetch();
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: _dec(
                      c,
                    ).copyWith(hintText: 'e.g. Paid via PhonePe'),
                  ),
                  const SizedBox(height: 16),
                  SizedBox(
                    width: double.infinity,
