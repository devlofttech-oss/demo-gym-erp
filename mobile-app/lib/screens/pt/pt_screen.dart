...
class _PTScreenState extends State<PTScreen>
  ...
  /// Completed sessions for a package, counted from the session records the
  /// way web does — no denormalised counter to drift out of sync.
  int _completedFor(String? packageId) {
    if (packageId == null) return 0;
    return _sessions
        .where((x) => x['packageId'] == packageId && x['status'] == 'Completed')
        .length;
  }
  ...
  double get _totalRevenue {
    ...
    return _packages
        .where(
          (p) => (p['createdAt'] ?? p['startDate'] ?? '').toString().startsWith(
            prefix,
          ),
        )
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () =>
                _tab.index == 0 ? _showPackageForm() : _showSessionForm(),
          ),
        ],
        bottom: TabBar(
          controller: _tab,
      ...
      body: RefreshIndicator(
        ...
        child: _loading
            ...
            : TabBarView(
                controller: _tab,
                children: [_packagesTab(c), _sessionsTab(c)],
              ),
      ),
    );
  }
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildBuilderDelegate(
                (_, i) => _PackageCard(
                  pkg: _packages[i],
                  sessionsCompleted: _completedFor(
                    _packages[i]['id'] as String?,
                  ),
...
class _PackageCard extends StatelessWidget {
  ...
  Color get _statusColor {
    final remaining =
        asNum(pkg['sessionsIncluded']).toInt() - sessionsCompleted;
  ...
  Widget build(BuildContext context) {
    ...
    final remaining = (sessionsIncluded - sessionsCompleted).clamp(
      0,
      sessionsIncluded,
    );
      ...
      child: KCard(
        child: Column(
          ...
          children: [
                  ...
                  child: ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      value: sessionsIncluded > 0
                          ? (sessionsCompleted / sessionsIncluded).clamp(
                              0.0,
                              1.0,
                            )
                          : 0,
            ...
            const SizedBox(height: 2),
            Text(
              '$sessionsCompleted / $sessionsIncluded sessions completed',
              style: KText.bodyMd.copyWith(color: c.onSurfaceVariant),
            ),
          ],
        ),
              ...
              itemBuilder: (_) => [
                ...
                if ((session['status'] ?? 'Scheduled') == 'Scheduled')
                  const PopupMenuItem(
                    ...
                    child: Text(
                      'Mark Completed',
                      style: TextStyle(color: TW.emerald600),
                    ),
                  ),
                const PopupMenuItem(
                  value: 'delete',
...
class _PackageFormState extends State<_PackageForm> {
  ...
  void initState() {
    ...
    if (pkg != null) {
      ...
      _priceCtrl.text = asNum(pkg['price']) == 0
          ? ''
          : asNum(pkg['price']).toString();
            ...
            if (widget.packages.isNotEmpty)
              DropdownButtonFormField<String>(
                ...
                onChanged: (v) => setState(() {
                  _packageId = v;
                  if (v != null) {
                    final pkg = widget.packages.firstWhere(
                      (p) => p['id'] == v,
                      orElse: () => {},
                    );
                    _trainerId = pkg['trainerId'] as String?;
