...
class _CheckinScreenState extends State<CheckinScreen> {
  ...
  // Older builds encoded member QRs as "kilos:member:<id>". Those codes are
  // already saved on members' phones, so accept either form.
  static const _legacyQrPrefix = 'kilos:member:';

  Future<void> _processCheckin(String raw) async {
    if (_checkingIn) return;
    final scannedId = raw.startsWith(_legacyQrPrefix)
        ? raw.substring(_legacyQrPrefix.length)
        : raw;
    final member = _members.where((m) => m['id'] == scannedId).firstOrNull;
    final staff = member == null
        ? _staff
              .where((s) => s['qrId'] == scannedId || s['id'] == scannedId)
              .firstOrNull
        : null;
  ...
  Future<void> _processMember(Map<String, dynamic> member) async {
    ...
    final todayRecords = await TenantDb.getCollection(
      gymId,
      'attendance',
      conditions: [
    ...
    final inGrace =
        hasBalance && nextPay != null && today.compareTo(nextPay) <= 0;
    ...
    if (active != null) {
      ...
      if (warnBalance) {
        Beep.warning();
        _toast(
          '${member['name']} checked in — Balance due: ${rupees(asNum(member['balanceFees']))}',
          TW.amber700,
        );
      } else if (inGrace) {
        Beep.checkin();
        _toast(
          '${member['name']} checked in — Balance due by $nextPay',
          TW.blue700,
        );
      } else {
        Beep.checkin();
      ...
      _push(
        _Entry(
          ...
          action: 'in',
          time: checkIn,
          balanceDue: warnBalance,
          gracePeriod: inGrace,
          balanceFees: asNum(member['balanceFees']),
  ...
  Future<void> _manualCheckin() async {
    if (_selectedMember == null) {
      _toast('Please select a member', TW.rose600);
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        Text(
          'Gym Check-in Scanner',
          style: KText.h1.copyWith(color: c.onSurface),
        ),
        const SizedBox(height: 6),
        Text(
          'Scan a QR to check in or out. Works for members and staff.',
          style: KText.bodyLg.copyWith(color: c.onSurfaceVariant),
        ),
    ...
    return KCard(
      child: Column(
        ...
        children: [
          ...
          Text(
            'Type member name or phone to search',
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Sym(
                  _selectedMember != null ? MSym.checkCircle : MSym.search,
                  size: 18,
                  color: _selectedMember != null
                      ? c.primary
                      : c.onSurfaceVariant,
                ...
                Expanded(
                  child: TextField(
                    ...
                    onTap: () {
                      if (_selectedMember != null) _clearManualSelection();
                    },
          ...
          SizedBox(
            ...
            child: FilledButton.icon(
              ...
              onPressed: (_checkingIn || _loading || _selectedMember == null)
                  ? null
                  : _manualCheckin,
              icon: _checkingIn
                  ? const KSpinner(size: 16, color: Colors.white)
                  : const Sym(MSym.howToReg, size: 16),
              label: Text(
                _checkingIn
                    ? 'Processing...'
                    : 'Check In${_selectedMember != null ? ' · ${_selectedMember!['name']}' : ''}',
              ),
            ),
          ),
        ],
  ...
  Widget _entryTile(_Entry e) {
    ...
    if (warn) {
      ...
      iconBg = TW.emerald100;
      iconFg = TW.emerald600;
      icon = MSym.login;
    }
    ...
    sub.write(
      out
          ? 'Check-out${e.duration != null ? ' · ${e.duration} min' : ''}'
          : 'Check-in',
    );
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Sym(icon, size: 22, color: iconFg, fill: true),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
          ...
          Text(
            fmtTime(e.time.toIso8601String()),
            style: TextStyle(
              color: c.onSurface,
              fontWeight: FontWeight.w700,
              fontSize: 13,
