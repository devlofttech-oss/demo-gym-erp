...
class _DashboardScreenState extends State<DashboardScreen> {
    ...
    try {
      ...
      _todayAttendance =
          attendance.where((a) {
            ...
            final db =
                toDate(b['checkInTime'] ?? b['timestamp']) ?? DateTime(1970);
      ...
      for (final a in attendance) {
        acts.add(
          _Activity(
            'checkin',
            '${a['memberName']} checked in',
            toDate(a['checkInTime'] ?? a['timestamp']),
          ),
        );
      }
      acts.sort(
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          _chartCard(
            'Revenue Trend',
            SizedBox(height: 220, child: _revenueTrendChart()),
          ),
          const SizedBox(height: 16),
          _chartCard(
            'Membership Status',
            SizedBox(height: 200, child: _membershipPie()),
          ),
          const SizedBox(height: 16),
          _chartCard(
            'Revenue vs Expenses (Last 6 Months)',
            SizedBox(height: 220, child: _revVsExpChart()),
          ),
          const SizedBox(height: 16),
          _chartCard(
            'Revenue by Plan',
    ...
    return Column(
      children: [
          ...
          children: [
            Expanded(
              child: _expenseCard(
                ...
                TW.rose100,
                'Total Expenses',
                _loading ? '...' : rupees(_totalExpenses),
                'All Time',
              ),
            ),
            ...
            Expanded(
              child: _expenseCard(
                ...
                TW.amber100,
                'Monthly Expenses',
                _loading ? '...' : rupees(_monthlyExpenses),
                'This Month',
              ),
            ),
          ],
        ...
        _expenseCard(
          ...
          _loading
              ? '...'
              : '${profitPos ? '' : '-'}${rupees(_netProfit.abs())}',
          'This Month',
    ...
    return KCard(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            value,
            style: KText.statValue.copyWith(color: valueColor ?? c.onSurface),
          ),
        ],
      ),
    );
        ...
        children: [
          ...
          if (_loading)
            ...
            const Center(
              child: KEmpty(
                icon: MSym.eventBusy,
                message: 'No check-ins recorded today yet.',
              ),
            )
          else ...[
            ...
            if (_todayAttendance.length > 5) ...[
              ...
              TextButton(
                ...
                child: Text(
                  'View all ${_todayAttendance.length} check-ins',
                  style: TextStyle(
                    color: c.primary,
                    fontWeight: FontWeight.w600,
                  ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  '${_expiringSoon.length} member${_expiringSoon.length > 1 ? 's' : ''} expiring within 7 days',
  ...
  Widget _revenueTrendChart() {
    final spots = [
      for (var i = 0; i < _revenueTrend.length; i++)
        FlSpot(i.toDouble(), _revenueTrend[i].y.toDouble()),
    ];
    return LineChart(
      LineChartData(
        ...
        titlesData: FlTitlesData(
          ...
          leftTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (v, _) => Text(
                '₹${v.toInt()}',
                style: const TextStyle(color: TW.slate400, fontSize: 10),
              ),
            ),
          ),
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              showTitles: true,
              interval: (_revenueTrend.length / 4).ceilToDouble().clamp(1, 999),
              getTitlesWidget: (v, _) {
                final i = v.toInt();
          ...
          LineChartBarData(
            ...
            belowBarData: BarAreaData(
              ...
              gradient: LinearGradient(
                ...
                colors: [
                  TW.violet600.withValues(alpha: 0.7),
                  TW.violet600.withValues(alpha: 0),
                ],
              ),
            ),
          ),
      ...
      children: [
        ...
        Column(
          ...
          children: [
            Text(
              '$_totalMembers',
              style: KText.statValue.copyWith(color: context.c.onSurface),
            ),
            Text(
              'Total',
              style: KText.labelCaps.copyWith(
  ...
  Widget _revVsExpChart() {
    final maxY = _revVsExp.fold<double>(
      0,
      (m, b) => [
        m,
        b.rev.toDouble(),
      ...
      BarChartData(
        ...
        gridData: FlGridData(
          ...
          getDrawingHorizontalLine: (_) => FlLine(
            color: TW.slate400.withValues(alpha: 0.15),
            strokeWidth: 1,
          ),
        ),
        borderData: FlBorderData(show: false),
        ...
        barGroups: [
          for (var i = 0; i < _revVsExp.length; i++)
            BarChartGroupData(
              ...
              barRods: [
                BarChartRodData(
                  toY: _revVsExp[i].rev.toDouble(),
                  color: TW.violet600,
                  width: 8,
                  borderRadius: BorderRadius.circular(2),
                ),
                BarChartRodData(
                  toY: _revVsExp[i].exp.toDouble(),
                  color: TW.rose500,
                  width: 8,
                  borderRadius: BorderRadius.circular(2),
                ),
  ...
  Widget _revByPlanChart() {
    final maxY = _revenueByPlan.fold<double>(
      0,
      (m, e) => e.y > m ? e.y.toDouble() : m,
    );
    return BarChart(
      BarChartData(
        maxY: maxY == 0 ? 10 : maxY * 1.2,
        ...
        titlesData: FlTitlesData(
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(
              ...
              getTitlesWidget: (v, _) {
                final i = v.toInt();
                if (i < 0 || i >= _revenueByPlan.length)
                ...
                return Padding(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(color: TW.slate400, fontSize: 9),
                  ),
                );
              },
            ),
        ...
        barGroups: [
          for (var i = 0; i < _revenueByPlan.length; i++)
            BarChartGroupData(
              ...
              barRods: [
                BarChartRodData(
                  toY: _revenueByPlan[i].y.toDouble(),
                  color: TW.violet600,
                  width: 16,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(
            'Recent Activity',
            style: KText.h3.copyWith(color: c.onSurface, fontSize: 17),
          ),
          const SizedBox(height: 16),
          if (_loading)
            const KLoading()
