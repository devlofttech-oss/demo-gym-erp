...
class _AttendanceScreenState extends State<AttendanceScreen> {
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    return _records.where((a) {
      ...
      if (_dateFilter == 'today')
        ...
        matchDate = d.compareTo(weekStart) >= 0 && d.compareTo(today) <= 0;
      else if (_dateFilter == 'month')
        matchDate = d.compareTo(monthStart) >= 0;
      else if (_dateFilter == 'custom')
        matchDate = d == _customDate;
      if (matchDate && _selectedDay != null) matchDate = d == _selectedDay;
      final matchTerm =
          term.isEmpty ||
          ((a['memberName'] as String?)?.toLowerCase().contains(term) ?? false);
  ...
  int get _totalToday =>
      _records.where((a) => recordDate(a) == todayStr()).length;
  int get _totalMonth {
    final monthStart = DateTime(
      DateTime.now().year,
      DateTime.now().month,
    ...
    return _records.where((a) {
      final d = recordDate(a);
      return d != null && d.compareTo(monthStart) >= 0;
    }).length;
  }
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            Text(
              'All member check-ins sorted by date.',
            ...
            if (_loading)
              ...
              const KEmpty(icon: MSym.eventBusy, message: 'No check-ins found')
            else
              _groupedList(paginated),
            if (!_loading && totalPages > 1)
              _pagination(totalPages, filtered.length),
          ],
        ),
      ),
    );
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: bg.withValues(alpha: context.isDark ? 0.2 : 1),
              ...
              onTap: () async {
                if (f.$1 == 'custom') {
                  final picked = await showDatePicker(
                    context: context,
                    initialDate:
                        DateTime.tryParse(_customDate) ?? DateTime.now(),
                    firstDate: DateTime(2020),
                    lastDate: DateTime(2100),
                  );
                  if (picked != null)
                    _customDate = picked.toIso8601String().split('T').first;
                }
                setState(() {
                  _dateFilter = f.$1;
                  _selectedDay = null;
      ...
      if (d != prevDate) {
        widgets.add(
          Padding(
            ...
            child: Text(
              d != null ? _dateLabel(d).toUpperCase() : 'UNKNOWN DATE',
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              (a['memberName'] as String?) ?? 'Unknown',
              style: TextStyle(
                color: c.onSurface,
                fontWeight: FontWeight.w500,
                fontSize: 14,
          ...
          Column(
            ...
            children: [
              ...
              if (out)
                Text(
                  '${fmtTime(a['checkOutTime'])} · ${fmtDuration(a['duration'])}',
                  style: const TextStyle(color: TW.rose500, fontSize: 12),
                )
              else
