...
class _MembersScreenState extends State<MembersScreen> {
  ...
  Future<void> _fetchAbsentees() async {
    ...
    try {
      ...
      final thirtyDaysAgo = now
          ...
          .split('T')
          .first;
      final recent = await TenantDb.getCollection(
        gymId,
        'attendance',
      ...
      for (final a in recent) {
        ...
        if (!lastVisit.containsKey(mid) || d.compareTo(lastVisit[mid]!) > 0)
          lastVisit[mid] = d;
      }
      final list = _members
          .where((m) {
          ...
          .map((m) => {...m, 'lastVisit': lastVisit[m['id']]})
          .toList();
      setState(() {
        _absentees = list;
        _absLoaded = true;
  ...
  bool _matchesCategory(Map<String, dynamic> m, String cat) {
    ...
    if (cat == 'Group Classes')
      return plan.startsWith('Group Classes') || plan.startsWith('Kids Dance');
  ...
  List<Map<String, dynamic>> get _filtered {
    ...
    return _members.where((m) {
      ...
      if (_status == 'Active')
        matchStatus = !isFrozen && (days == null || days >= 0);
      else if (_status == 'Expired')
        matchStatus = !isFrozen && days != null && days < 0;
      else if (_status == 'Expiring')
        matchStatus = !isFrozen && days != null && days >= 0 && days <= 7;
      else if (_status == 'Frozen')
        matchStatus = isFrozen;
      else if (_status == 'Balance Due')
        matchStatus = asNum(m['balanceFees']) > 0;
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          PageHeader(
            'Members',
            'Manage your gym members, plans, and statuses.',
            trailing: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                if (context.read<AuthProvider>().role != 'staff') ...[
                  _importBtn(),
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Sym(MSym.uploadFile, size: 18, color: c.primary),
        ),
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              '$_expiringCount member${_expiringCount > 1 ? 's' : ''} expiring within 7 days — send a WhatsApp reminder with the Remind button in their row.',
    ...
    return SingleChildScrollView(
      ...
      child: Row(
        children: cats.map((cat) {
          ...
          final count = cat == 'All'
              ? _members.length
              : _members.where((m) => _matchesCategory(m, cat)).length;
          final m = meta[cat];
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(
                        cat,
                        style: TextStyle(
                          color: active ? Colors.white : c.onSurfaceVariant,
                          fontWeight: FontWeight.w500,
                          fontSize: 14,
                        ),
                      ),
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: active ? Colors.white24 : c.surfaceContainer,
            ...
            child: Material(
              ...
              child: InkWell(
                ...
                onTap: () {
                  ...
                  if (f.$1 == 'Absentees' && !_absLoaded && _members.isNotEmpty)
                    _fetchAbsentees();
                },
                child: Container(
                  padding: const EdgeInsets.symmetric(
    ...
    return Row(
      children: [
        ...
        Expanded(
          child: Text(
            v,
            style: TextStyle(color: c.onSurface, fontWeight: FontWeight.w500),
  ...
  Widget _absenteesView() {
    final c = context.c;
    if (_absLoading)
      return const KLoading(label: 'Checking attendance records…');
    if (_absentees.isEmpty) {
      return const KEmpty(
        icon: MSym.sentimentSatisfied,
        iconColor: TW.emerald500,
        message: 'No absentees — everyone has visited in the last 5 days!',
      );
    }
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
          children: [
            ...
            Expanded(
              child: Text(
                '${_absentees.length} active member${_absentees.length != 1 ? 's' : ''} absent for 5+ days',
                style: TextStyle(
                  color: c.onSurface,
                  fontWeight: FontWeight.w600,
                ),
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          since != null
                              ? 'Last visit: $since day${since != 1 ? 's' : ''} ago ($lv)'
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            'Page $_page of $totalPages  ($total members)',
