...
import '../leads/leads_screen.dart'
    show canonLead, leadStatusLabel, leadSourceLabel;
...
class _ReportScreenState extends State<ReportScreen>
    ...
    try {
      ...
      if (mounted) {
        ...
        setState(() {
          ...
          _prevMembers = res[1]
              .where(
                (m) => (m['joinDate'] as String? ?? '').startsWith(prevPrefix),
              )
              .toList();
  ...
  // Computed stats
  double get _revenue => _payments.fold(0.0, (s, p) => s + asNum(p['amount']));
  ...
  double get _prevRevenue =>
      _prevPayments.fold(0.0, (s, p) => s + asNum(p['amount']));
  int get _prevNewMembers => _prevMembers.length;
  int get _prevRenewals =>
      _prevPayments.where((p) => (p['type'] ?? '') == 'Renewal').length;
  ...
  double get _conversionRate =>
      _leads.isEmpty ? 0 : _leadsWon / _leads.length * 100;
  ...
  Map<String, int> get _leadsByStatus {
    ...
    for (final l in _leads) {
      final s = leadStatusLabel(l['status'] as String? ?? 'new');
      map[s] = (map[s] ?? 0) + 1;
    }
    return map;
  }
  ...
  Map<String, int> get _leadsBySource {
    ...
    for (final l in _leads) {
      final s = leadSourceLabel(l['source'] as String? ?? 'other');
      map[s] = (map[s] ?? 0) + 1;
    }
    return map;
  }
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      await Printing.sharePdf(
        bytes: bytes,
        filename: 'kilos_report_$_monthPrefix.pdf',
      );
    } catch (_) {}
                                ...
                                child: BarChart(
                                  BarChartData(
                                    gridData: FlGridData(
                                      ...
                                      horizontalInterval: (_revenue / 4).clamp(
                                        1,
                                        double.infinity,
                                      ),
          ...
          if (_loading)
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ...bySource.entries.map(
                          (e) =>
                              _ProgressRow(e.key, e.value, total, TW.violet600),
                        ),
                      ],
                    ),
                  ),
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Text(
                  'Leads this month (${_leads.length})',
                            ...
                            Column(
                              ...
                              children: [
                                Pill(
                                  status,
                                  fg: sc,
                                  bg: sc.withValues(alpha: 0.1),
                                  dot: true,
                                ),
                                if ((l['source'] as String? ?? '')
                                    .isNotEmpty) ...[
                                  const SizedBox(height: 4),
                                  Text(
                                    l['source'] as String,
      ...
      child: Row(
        children: [
          SizedBox(
            ...
            child: Text(
              label,
              style: const TextStyle(color: TW.slate500, fontSize: 12),
            ),
          ),
          Expanded(
            child: ClipRRect(
